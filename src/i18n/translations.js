export const translations = {
  pt: {
    appTitle: 'Photos Page',
    subtitle: 'Adicione, organize e visualize as folhas do álbum em PDF',
    persistHint: 'O álbum e as fotos ficam guardados neste browser',
    currentAlbum: 'Álbum',
    newAlbum: 'Novo',
    deleteAlbum: 'Apagar álbum',
    renameAlbum: 'Nome do álbum',
    confirmDeleteAlbumTitle: 'Apagar este álbum?',
    confirmDeleteAlbumMessage:
      'O álbum e as respetivas fotos serão removidos. Não pode ser desfeito.',
    confirmDeleteAlbumYes: 'Sim, apagar álbum',
    rotatePhoto: 'Rodar 90°',
    setAsCover: 'Usar como capa',
    captionPlaceholder: 'Legenda',
    dropOnPage: 'Largar aqui',
    albumMoveHint: 'Arraste uma foto para outra folha ou para a capa',
    coverSheet: 'Capa',
    coverShort: 'Capa',
    coverTitlePlaceholder: 'Título do álbum',
    coverDatePlaceholder: 'Data ou subtítulo',
    coverEmptyHint: 'Arraste uma foto para a capa ou toque em ★',
    loading: 'A carregar fotos guardadas...',
    tabEntry: 'Entrada de Fotos',
    tabList: 'Lista de Fotos',
    tabAlbum: 'Álbum',
    entryTitle: 'Entrada de Fotos',
    entryDesc:
      'Escolha uma ou várias imagens. Remova as que não quiser (✕) e depois adicione à coleção.',
    dropHint: 'Clique ou arraste imagens (pode escolher várias)',
    pendingCount: (n) =>
      `${n} foto${n !== 1 ? 's' : ''} selecionada${n !== 1 ? 's' : ''}`,
    clearPending: 'Limpar tudo',
    removeFromSelection: 'Remover desta seleção',
    addToCollection: 'Adicionar à coleção',
    addPhotos: (n) => `Adicionar ${n} foto${n !== 1 ? 's' : ''}`,
    addedHint: (n) =>
      `${n} foto${n !== 1 ? 's' : ''} adicionada${n !== 1 ? 's' : ''} à coleção. Pode escolher mais.`,
    listTitle: 'Lista de Fotos',
    listEmpty: 'Ainda não há fotos. Adicione na secção de entrada.',
    selectedOf: (n, total) =>
      `${n} de ${total} selecionada${n !== 1 ? 's' : ''}`,
    dragToReorder: 'Arraste as fotos para definir a ordem do álbum',
    albumListHint: 'Selecione as fotos (✓) e abra o separador Álbum para ver as folhas',
    dragHandle: 'Arrastar para reordenar',
    selectAll: 'Selecionar todas',
    deselectAll: 'Desselecionar todas',
    removePhoto: 'Remover foto',
    selectForPrint: 'Selecionar para imprimir',
    deselectPhoto: 'Desselecionar',
    clearAll: 'Limpar tudo',
    confirmClearTitle: 'Limpar todas as fotos?',
    confirmClearMessage:
      'Esta ação remove todas as fotos da coleção. Não pode ser desfeita.',
    confirmYes: 'Sim, limpar tudo',
    confirmNo: 'Cancelar',
    removeSelected: 'Apagar selecionadas',
    confirmRemoveSelectedTitle: (n) =>
      `Apagar ${n} foto${n !== 1 ? 's' : ''} selecionada${n !== 1 ? 's' : ''}?`,
    confirmRemoveSelectedMessage:
      'As fotos selecionadas serão removidas da coleção. Não pode ser desfeito.',
    confirmRemoveSelectedYes: 'Sim, apagar',
    selectPhotosAlertTitle: 'Nenhuma foto selecionada',
    selectPhotosAlertMessage:
      'Selecione pelo menos uma foto na lista (✓) antes de guardar o PDF.',
    selectPhotosAlertOk: 'OK',
    emailSection: 'Enviar por email',
    emailRecipient: 'Email da gráfica',
    emailRecipientPlaceholder: 'grafica@exemplo.com',
    emailMessage: 'Mensagem (opcional)',
    emailMessagePlaceholder: 'Pedido de impressão das fotos em anexo.',
    emailHint:
      'No telemóvel ou em alguns PCs, abre o email com o anexo. Caso contrário, descarrega o ficheiro e abre o seu programa de email — anexe o PDF/ZIP e envie.',
    sendEmail: 'Enviar por email',
    sendingEmail: 'A preparar email...',
    emailSubject: (name) => `Pedido de impressão — ${name}`,
    emailBodyDefault: 'Segue em anexo o PDF com as fotos para impressão.',
    emailAttachHint:
      'Anexe o ficheiro que acabou de ser descarregado a este email antes de enviar.',
    emailSuccessShare: 'Escolha a app de email para concluir o envio.',
    emailSuccessMailto:
      'O ficheiro foi descarregado e o seu email abriu — anexe o PDF/ZIP e envie.',
    emailNoRecipient: 'Indique o email da gráfica.',
    emailInvalid: 'Email inválido.',
    emailCancelled: 'Envio cancelado.',
    printTitle: 'Álbum',
    printDesc:
      'Veja cada folha, ajuste o tamanho das fotos para preencher o espaço vazio e exporte o PDF',
    albumEmpty: 'Selecione fotos na lista (✓) para ver as folhas do álbum',
    albumPageOf: (current, total) => `Folha ${current} de ${total}`,
    albumSheet: (n) => `Folha ${n}`,
    albumFillRatio: (pct) => `${pct}% da folha preenchida`,
    albumPrev: 'Folha anterior',
    albumNext: 'Folha seguinte',
    albumSheets: 'Folhas do álbum',
    albumPageInclude: 'Vai para o PDF',
    albumPageExclude: 'Não vai para o PDF',
    noPagesAlertTitle: 'Nenhuma folha incluída',
    noPagesAlertMessage:
      'Pelo menos uma folha precisa do visto verde para ser incluída no PDF.',
    printSummarySkipped: (photos, included, total) =>
      `${photos} foto${photos !== 1 ? 's' : ''} → ${included} de ${total} folhas no PDF`,
    layoutMode: 'Tamanho das fotos',
    layoutModeFill: 'Preencher espaço',
    layoutModeGrid: 'Grelha',
    layoutModeHint_fill:
      'As fotos ficam com tamanho semelhante e ocupam a folha de forma equilibrada',
    layoutModeHint_grid:
      'Tamanhos iguais numa grelha clássica — pode sobrar mais espaço em branco',
    quality: 'Qualidade',
    qualityEmail: 'Email',
    qualityPrint: 'Impressão',
    qualityHint_email:
      'Ficheiro mais leve (~200 DPI) — ideal para enviar por email (limite 25 MB)',
    qualityHint_print:
      'Alta qualidade (~300 DPI) — melhor para impressão profissional',
    estimatingSize: 'A calcular tamanho estimado...',
    sizeEstimateOk: (size) => `~${size} — OK para email`,
    sizeEstimateSplit: (size, parts) =>
      `~${size} — será dividido em ${parts} PDF${parts !== 1 ? 's' : ''}`,
    downloadAsZip: 'Descarregar como ZIP (vários PDFs num ficheiro)',
    generatingPart: (current, total) =>
      `A gerar parte ${current} de ${total}...`,
    orientation: 'Orientação da página',
    portrait: 'Retrato',
    landscape: 'Paisagem',
    photosPerPage: 'Fotos por página',
    filename: 'Nome do ficheiro',
    printSummary: (photos, pages) =>
      `${photos} foto${photos !== 1 ? 's' : ''} → ${pages} página${pages !== 1 ? 's' : ''}`,
    savePdf: 'Guardar PDF',
    generatingPdf: 'A gerar PDF...',
    noPhotosSelected: 'Selecione pelo menos uma foto para imprimir',
    pdfError: 'Erro ao gerar PDF',
    orientationShort: { portrait: 'retrato', landscape: 'paisagem' },
    layouts: {
      1: '1 foto — página inteira',
      2: '2 fotos — lado a lado',
      3: '3 fotos — fila horizontal',
      4: '4 fotos — grelha 2×2',
      5: '5 fotos — 3 em cima, 2 centradas em baixo',
      6: '6 fotos — 2 por fila (3 filas)',
      7: '7 fotos — 4 em cima, 3 centradas em baixo',
      8: '8 fotos — grelha 4×2',
    },
  },
  en: {
    appTitle: 'Photos Page',
    subtitle: 'Add, organize and preview your album pages as PDF',
    persistHint: 'The album and photos are saved in this browser',
    currentAlbum: 'Album',
    newAlbum: 'New',
    deleteAlbum: 'Delete album',
    renameAlbum: 'Album name',
    confirmDeleteAlbumTitle: 'Delete this album?',
    confirmDeleteAlbumMessage:
      'The album and its photos will be removed. This cannot be undone.',
    confirmDeleteAlbumYes: 'Yes, delete album',
    rotatePhoto: 'Rotate 90°',
    setAsCover: 'Use as cover',
    captionPlaceholder: 'Caption',
    dropOnPage: 'Drop here',
    albumMoveHint: 'Drag a photo onto another page or the cover',
    coverSheet: 'Cover',
    coverShort: 'Cover',
    coverTitlePlaceholder: 'Album title',
    coverDatePlaceholder: 'Date or subtitle',
    coverEmptyHint: 'Drag a photo onto the cover or tap ★',
    loading: 'Loading saved photos...',
    tabEntry: 'Photo Entry',
    tabList: 'Photo List',
    tabAlbum: 'Album',
    entryTitle: 'Photo Entry',
    entryDesc:
      'Choose one or more images. Remove any you do not want (✕), then add to the collection.',
    dropHint: 'Click or drag images (you can choose several)',
    pendingCount: (n) => `${n} photo${n !== 1 ? 's' : ''} selected`,
    clearPending: 'Clear all',
    removeFromSelection: 'Remove from selection',
    addToCollection: 'Add to collection',
    addPhotos: (n) => `Add ${n} photo${n !== 1 ? 's' : ''}`,
    addedHint: (n) =>
      `${n} photo${n !== 1 ? 's' : ''} added to the collection. You can choose more.`,
    listTitle: 'Photo List',
    listEmpty: 'No photos yet. Add some in the entry section.',
    selectedOf: (n, total) => `${n} of ${total} selected`,
    dragToReorder: 'Drag photos to set the album order',
    albumListHint: 'Select photos (✓) and open the Album tab to preview the pages',
    dragHandle: 'Drag to reorder',
    selectAll: 'Select all',
    deselectAll: 'Deselect all',
    removePhoto: 'Remove photo',
    selectForPrint: 'Select for printing',
    deselectPhoto: 'Deselect',
    clearAll: 'Clear all',
    confirmClearTitle: 'Clear all photos?',
    confirmClearMessage:
      'This will remove all photos from the collection. This cannot be undone.',
    confirmYes: 'Yes, clear all',
    confirmNo: 'Cancel',
    removeSelected: 'Delete selected',
    confirmRemoveSelectedTitle: (n) =>
      `Delete ${n} selected photo${n !== 1 ? 's' : ''}?`,
    confirmRemoveSelectedMessage:
      'Selected photos will be removed from the collection. This cannot be undone.',
    confirmRemoveSelectedYes: 'Yes, delete',
    selectPhotosAlertTitle: 'No photos selected',
    selectPhotosAlertMessage:
      'Select at least one photo in the list (✓) before saving the PDF.',
    selectPhotosAlertOk: 'OK',
    emailSection: 'Send by email',
    emailRecipient: 'Print shop email',
    emailRecipientPlaceholder: 'printshop@example.com',
    emailMessage: 'Message (optional)',
    emailMessagePlaceholder: 'Print request with photos attached.',
    emailHint:
      'On mobile or some PCs, opens email with the attachment. Otherwise, downloads the file and opens your email app — attach the PDF/ZIP and send.',
    sendEmail: 'Send by email',
    sendingEmail: 'Preparing email...',
    emailSubject: (name) => `Print request — ${name}`,
    emailBodyDefault: 'Please find attached the PDF with photos for printing.',
    emailAttachHint:
      'Attach the file that was just downloaded to this email before sending.',
    emailSuccessShare: 'Choose your email app to complete sending.',
    emailSuccessMailto:
      'The file was downloaded and your email app opened — attach the PDF/ZIP and send.',
    emailNoRecipient: 'Enter the print shop email.',
    emailInvalid: 'Invalid email address.',
    emailCancelled: 'Send cancelled.',
    printTitle: 'Album',
    printDesc:
      'Preview each page, size photos to fill empty space, and export the PDF',
    albumEmpty: 'Select photos in the list (✓) to preview album pages',
    albumPageOf: (current, total) => `Page ${current} of ${total}`,
    albumSheet: (n) => `Page ${n}`,
    albumFillRatio: (pct) => `${pct}% of the page filled`,
    albumPrev: 'Previous page',
    albumNext: 'Next page',
    albumSheets: 'Album pages',
    albumPageInclude: 'Goes to the PDF',
    albumPageExclude: 'Will not go to the PDF',
    noPagesAlertTitle: 'No pages included',
    noPagesAlertMessage:
      'At least one page needs the green check to be included in the PDF.',
    printSummarySkipped: (photos, included, total) =>
      `${photos} photo${photos !== 1 ? 's' : ''} → ${included} of ${total} pages in the PDF`,
    layoutMode: 'Photo size',
    layoutModeFill: 'Fill space',
    layoutModeGrid: 'Grid',
    layoutModeHint_fill:
      'Photos stay a similar size and share the page evenly',
    layoutModeHint_grid:
      'Equal sizes in a classic grid — more leftover whitespace is likely',
    quality: 'Quality',
    qualityEmail: 'Email',
    qualityPrint: 'Print',
    qualityHint_email:
      'Smaller file (~200 DPI) — ideal for email (25 MB limit)',
    qualityHint_print:
      'High quality (~300 DPI) — best for professional printing',
    estimatingSize: 'Calculating estimated size...',
    sizeEstimateOk: (size) => `~${size} — OK for email`,
    sizeEstimateSplit: (size, parts) =>
      `~${size} — will be split into ${parts} PDF${parts !== 1 ? 's' : ''}`,
    downloadAsZip: 'Download as ZIP (multiple PDFs in one file)',
    generatingPart: (current, total) =>
      `Generating part ${current} of ${total}...`,
    orientation: 'Page orientation',
    portrait: 'Portrait',
    landscape: 'Landscape',
    photosPerPage: 'Photos per page',
    filename: 'File name',
    printSummary: (photos, pages) =>
      `${photos} photo${photos !== 1 ? 's' : ''} → ${pages} page${pages !== 1 ? 's' : ''}`,
    savePdf: 'Save PDF',
    generatingPdf: 'Generating PDF...',
    noPhotosSelected: 'Select at least one photo to print',
    pdfError: 'Error generating PDF',
    orientationShort: { portrait: 'portrait', landscape: 'landscape' },
    layouts: {
      1: '1 photo — full page',
      2: '2 photos — side by side',
      3: '3 photos — horizontal row',
      4: '4 photos — 2×2 grid',
      5: '5 photos — 3 on top, 2 centered below',
      6: '6 photos — 2 per row (3 rows)',
      7: '7 photos — 4 on top, 3 centered below',
      8: '8 photos — 4×2 grid',
    },
  },
}
